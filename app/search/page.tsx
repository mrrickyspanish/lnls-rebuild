...
export default function SearchPage() {
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    setLoading(true);
    try {
      // TODO: Implement actual search when Sanity is connected
  ...
  const filteredResults = results.filter(result => {
    ...
    return result.type === activeTab.slice(0, -1); // 'articles' -> 'article'
  });
          ...
          <div className="flex gap-2 mb-8 border-b border-slate-800">
            ...
            >
              ...
              <span className="ml-2 text-xs text-slate-500">
                ({results.filter(r => r.type === 'article').length})
              </span>
              {activeTab === 'articles' && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-400"></div>
              )}
            ...
            >
              Episodes
              <span className="ml-2 text-xs text-slate-500">
                ({results.filter(r => r.type === 'episode').length})
              </span>
              {activeTab === 'episodes' && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-400"></div>
              )}
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            {result.category && (
              <span className="text-slate-500 text-sm">in {result.category}</span>
            )}
          </div>
  ...
  return (
    <div className="text-center py-16">
      ...
      <p className="text-slate-400 mb-8 max-w-md mx-auto">
        Find articles, episodes, and analysis across all Lakers and NBA coverage.
      </p>
