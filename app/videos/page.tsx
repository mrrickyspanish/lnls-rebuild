  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 py-8 md:py-12">
      <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-8">
        ...
        {/* Featured Video with scale-up animation */}
        <ScrollReveal direction="up" delay={0.2}>
          <div className="mb-12 bg-slate-900/40 backdrop-blur-sm rounded-3xl p-4 md:p-6 shadow-2xl">
            ...
            >
              ...
              <motion.div className="absolute inset-0 bg-black/40 group-hover:bg-black/60 transition-colors flex items-center justify-center">
                <motion.div 
                  whileHover={{ scale: 1.15 }}
                  className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-red-600 group-hover:bg-red-500 flex items-center justify-center shadow-2xl"
                >
        ...
        <ScrollReveal direction="left" delay={0.1}>
          <div className="mb-8 flex flex-wrap gap-2 rounded-full bg-slate-900/60 backdrop-blur-sm p-1 w-fit">
            {(["all", "news", "recaps", "analysis"] as FilterType[]).map((f, index) => {
        ...
        >
          {filtered.map((video) => {
            ...
            return (
              <StaggerItem key={video.id} variants={staggerItemVariants}>
                <motion.div
                  whileHover={{ y: -8 }}
                  transition={{ duration: 0.3 }}
                ...
                >
                  <div className="bg-slate-900/60 backdrop-blur-sm rounded-2xl overflow-hidden border border-slate-700/50 hover:border-indigo-500/50 transition-all hover:shadow-xl hover:shadow-indigo-500/20 h-full flex flex-col">
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                      <h3 className="text-base font-semibold text-white mb-2 line-clamp-2 group-hover:text-indigo-300 transition-colors flex-1">
                        {video.title}
                      </h3>
                      {video.published_at && (
                        <p className="text-xs text-slate-500">{formatDate(video.published_at)}</p>
                      )}
                    </div>
                  </div>
                </motion.div>
