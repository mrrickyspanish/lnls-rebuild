  ...
  try {
    ...
    const episodes: Episode[] = items.map((item, index) => {
      const getTag = (tag: string) => {
        ...
        // Then strip all HTML tags (including the decoded ones)
      ...
      // Parse duration (convert HH:MM:SS or MM:SS to seconds)
      let duration = 0;
      if (durationStr) {
      ...
      // Auto-detect topics based on title/description
      ...
      return {
        ...
        description,
        audio_url: audioUrl,
        image_url: imageUrl || 'https://images.unsplash.com/photo-1546519638-68e109498ffc?w=400&h=400&fit=crop',
    ...
    // Filter out episodes without title/audio and sort by published date (newest first)
    ...
    console.error("Spreaker RSS parsing error:", error);
    throw error;
  }
}
...
export async function GET() {
  try {
    ...
    const sampleEpisodes = [
      {
        ...
        description: 'Deep dive into the Lakers current roster construction, trade possibilities, and championship aspirations for this season.',
    ...
    return NextResponse.json(sampleEpisodes);
  }
}